...
if (!function_exists('settingsKeys')) {
    function settingsKeys()
    {
        return $settingsKeys = [
            ...
            'FROM_EMAIL' => "",
            'FROM_NAME' => "",
            'trainer_number_prefix' => "#TRNR-000",
            'trainee_number_prefix' => "#TRNE-000",
...
if (!function_exists('assignSubscription')) {
    function assignSubscription($id)
    {
        ...
        if ($subscription) {
            ...
            $users = User::where('parent_id', '=', parentId())->whereNotIn('type', ['trainer', 'trainee'])->get();
...
if (!function_exists('trainerPrefix')) {
    function trainerPrefix()
    {
        $settings = settings();
        return $settings["trainer_number_prefix"];
    }
}

if (!function_exists('traineePrefix')) {
    function traineePrefix()
    {
        $settings = settings();
        return $settings["trainee_number_prefix"];
    }
}
...
if (!function_exists('defaultTrainerCreate')) {
    function defaultTrainerCreate($id)
    {
        // Default Trainer role
        $trainerRoleData = [
            'name' => 'trainer',
            'parent_id' => $id,
        ];
        $systemTrainerRole = Role::create($trainerRoleData);
        // Default Trainer All Permissions
        $systemTrainerPermission = [
            ['name' => 'manage contact'],
            ['name' => 'create contact'],
            ['name' => 'edit contact'],
            ['name' => 'delete contact'],
        ...
        $systemTrainerRole->givePermissionTo($systemTrainerPermission);
    }
}

if (!function_exists('defaultTraineeCreate')) {
    function defaultTraineeCreate($id)
    {
        ...
        $systemTraineeRole = Role::create($traineeRoleData);
        // Default Trainee All Permissions
        $systemTraineePermission = [
            ['name' => 'manage contact'],
            ['name' => 'create contact'],
            ['name' => 'edit contact'],
            ['name' => 'delete contact'],
        ...
        $systemTraineeRole->givePermissionTo($systemTraineePermission);
    }
}
